...
- **三层内容体系**：中英缩写（OKR/GMV）→ 黑话行话（抓手/闭环）→ 业务与研发概念（埋点/灰度/提测），外加职场文化（双月会/背锅/画饼），按新人真实困惑组织。
- **结构化词条**：每个词条都有「一句话人话解释 + 详细解释 + 真实会议室例句 + 常见误解 + 相关词条」，不是一坨文字。
...
│   ├── business.json      # 业务与增长：埋点、A/B 测试、北极星指标……
...
1. **收词**：[发一个「新增词条」Issue](https://github.com/your-name/shuorenhua/issues/new/choose)，填 4 个字段即可。
